...
import it.pagopa.pdnd.interop.uservice.purposemanagement.model.persistence.PurposeVersionSuspended
...
class PurposeVersionSuspendedSerializer extends SerializerWithStringManifest {
  ...
  final val PurposeVersionSuspendedManifest: String = classOf[PurposeVersionSuspended].getName

  override def toBinary(o: AnyRef): Array[Byte] = o match {
    case event: PurposeVersionSuspended =>
      serialize(event, PurposeVersionSuspendedManifest, currentVersion)
  }

  override def fromBinary(bytes: Array[Byte], manifest: String): AnyRef = manifest.split('|').toList match {
    case PurposeVersionSuspendedManifest :: `version1` :: Nil =>
